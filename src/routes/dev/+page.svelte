<script>
    ...
    async function startScoring() {
        ...
        for (const [league, matchweek] of Object.entries(matchweeks)) {
            const maxGames = leagueMaxGames[league];
...
<div class="admin-container">
                ...
                {#each Object.entries(matchweeks) as [league, matchweek]}
                    <div class="matchweek-item">
                        <label for="mw-{league}">
                            ...
                            <span class="max-games">Max: {leagueMaxGames[league]}</span>
                        </label>
                        <input 
                            type="number" 
                            id="mw-{league}"
                            bind:value={matchweeks[league]} 
                            min="1" 
                            max={leagueMaxGames[league]}
                            disabled={isRunning}
                        />
                    </div>
                {/each}
    ...
    <div class="season-weights">
        ...
        <div class="weights-grid">
            {#each Object.entries(matchweeks) as [league, matchweek]}
                {@const totalGames = leagueMaxGames[league]}
